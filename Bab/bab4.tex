...
\For{$i \gets 0$ \textbf{to} \texttt{attrSplitConf.length}}	
	\If{\texttt{attrSplitConf[i].split(",")[2].equals(}\texttt{DISCRETE}\_\texttt{TYPE)}}
		...
		\EndFor
	\ElsIf{\texttt{attrSplitConf[i].split(",")[2].equals(}\texttt{NUMERIC}\_\texttt{TYPE})}
		\For{$j \gets 0$ \textbf{to} \texttt{classSplitConf.length}}
